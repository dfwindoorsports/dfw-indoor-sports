...
const SportsShowcase: React.FC = () => {
 return (
 <section className="bg-[#050911] border-y border-gray-200 transition-colors duration-300" id="sports-showcase">
 {sportsData.map((sport, index) => (
 <motion.div 
 key={sport.id} 
 transition={{ duration: 0.6 }}
 className={`py-24 ${index % 2 !== 0 ? 'bg-[#0A111F]' : 'bg-gray-50 dark:bg-[#0A111F]'} transition-colors duration-300`}
 >
 ...
 <div className="relative rounded-lg overflow-hidden shadow-2xl aspect-[4/3] border border-white/10">
 ...
 <h3 className="text-4xl md:text-5xl font-header font-bold text-white uppercase mb-6">
 {sport.title}
 </h3>
 ...
 <div>
 <h4 className="font-bold text-white text-xs uppercase tracking-widest mb-4 border-b border-white/10 pb-2">Highlights</h4>
 ...
 <div>
 <h4 className="font-bold text-white text-xs uppercase tracking-widest mb-4 border-b border-white/10 pb-2">Ways to Play</h4>
 <ul className="space-y-2">
 ...
 <Link href={sport.link} className="px-8 py-3 border border-gray-300 text-white font-bold uppercase tracking-widest text-xs rounded-sm hover:bg-dfw-navy dark:hover:bg-white hover:text-white dark:hover:text-dfw-navy transition-colors flex items-center justify-center gap-2 group">
