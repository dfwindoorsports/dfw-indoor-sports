...
const AboutSportsGrid: React.FC = () => {
 return (
 <section className="bg-[#0A111F] py-24 border-b border-white/5 relative overflow-hidden transition-colors duration-300">
 ...
 <h2 className="text-4xl md:text-5xl font-header font-bold text-white uppercase mb-6">Seven Sports. One Community.</h2>
 <p className="text-gray-400">Specialization without compromise. We embrace four key disciplines with professional focus.</p>
 </motion.div>
 ...
 : 'bg-white/5 border-white/10 text-white hover:shadow-lg hover:-translate-y-1'
 }`}
 >
 <div className="relative z-10">
 <div className="flex justify-between items-start mb-4">
 <sport.icon size={24} className={sport.featured ? 'text-white' : 'text-dfw-red'} />
 <span className={`text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded-sm ${sport.featured ? 'bg-black/20 text-white' : 'bg-gray-100 dark:bg-white/10 text-gray-500 '}`}>{sport.sub}</span>
 </div>
 <h3 className={`text-xl font-header font-bold uppercase mb-2 ${sport.featured ? 'text-white' : 'text-white'}`}>{sport.title}</h3>
 <p className={`text-xs leading-relaxed ${sport.featured ? 'text-white/80' : 'text-gray-400'}`}>{sport.desc}</p>
 </div>
 </motion.div>
 ))}
 </div>
