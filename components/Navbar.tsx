...
export default function Navbar() {
 ...
 ? 'bg-dfw-navy border-white/10'
 : scrolled
 ? 'bg-dfw-navy/95 backdrop-blur-xl border-white/10 shadow-md'
 ...
 <>
 <button
 className={`relative px-3 py-2 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-widest transition-colors duration-300 group-hover/menu:text-dfw-red ${pathname?.startsWith(item.href) ? 'text-white' : 'text-gray-400'
 }`}
 >
 {item.label}
 ...
 <div className="bg-[#0A111F] border border-white/10 shadow-xl rounded-sm overflow-hidden p-1">
 {item.children.map((child) => (
 <Link
 key={child.label}
 href={child.href}
 className="block px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-500 hover:text-dfw-red dark:hover:text-dfw-red hover:bg-gray-50 dark:hover:bg-white/5 transition-all rounded-sm flex items-center justify-between group/item"
 >
 ...
 <Link
 href={item.href}
 className={`relative px-3 py-2 block text-[11px] font-bold uppercase tracking-widest transition-colors duration-300 hover:text-dfw-red ${pathname === item.href ? 'text-white' : 'text-gray-400'
 }`}
 >
 {item.label}
 ...
 className="p-2.5 -mr-2 text-white hover:text-dfw-red transition-colors focus:outline-none"
 ...
 className="lg:hidden fixed inset-0 z-[90] bg-dfw-navy"
 >
 ...
 className="block font-header font-bold text-xl text-white uppercase"
 >
 {item.label}
 </Link>
 <button
 onClick={() => toggleExpanded(item.label)}
 className="p-2 text-white hover:text-dfw-red transition-colors"
 ...
 >
 <div className="pl-4 border-l-2 border-white/10 space-y-2 pb-2">
 {item.children.map(child => (
 <Link
 key={child.label}
 href={child.href}
 onClick={() => setIsOpen(false)}
 className="block text-xs font-bold text-gray-400 uppercase tracking-widest hover:text-dfw-red transition-colors py-1"
 >
 {child.label}
 </Link>
 ))}
 ...
 <Link
 href={item.href}
 className="block py-3 font-header font-bold text-xl text-white uppercase hover:text-dfw-red transition-colors"
 onClick={() => setIsOpen(false)}
 >
 {item.label}
 </Link>
 ...
 <a href="https://instagram.com/dfwindoorsports/" target="_blank" rel="noreferrer" className="hover:text-white transition-colors">Instagram</a>
 <a href="https://facebook.com/dfwindoorsports" target="_blank" rel="noreferrer" className="hover:text-white transition-colors">Facebook</a>
 <a href="https://twitter.com/dfwindoorsports" target="_blank" rel="noreferrer" className="hover:text-white transition-colors">Twitter</a>
 </div>
 </motion.div>
 </div>
 </motion.div>
