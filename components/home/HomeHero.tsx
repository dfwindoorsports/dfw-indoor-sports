...
import { ChevronRight, Calendar, MapPin, Zap, Car } from 'lucide-react';
  ...
  return (
    <div className="relative w-full h-[85vh] md:h-screen min-h-[500px] md:min-h-[700px] bg-[#020408] pb-6">
      ...
      <div className="relative h-full w-full overflow-hidden rounded-b-2xl md:rounded-b-[3rem] shadow-[0_40px_80px_-20px_rgba(10,17,31,0.6)] z-20">
        ...
        <div className="absolute inset-0 overflow-hidden bg-dfw-navy">
          ...
          <div className="absolute inset-0 opacity-50 animate-hero-zoom">
            <Image
              ...
              alt="Indoor cricket, badminton, soccer, and dodgeball at DFW Indoor Sports in Fort Worth Texas"
              fill
              priority
              fetchPriority="high"
              sizes="100vw"
        ...
        {/* Business Hours Badge - desktop only */}
        <div className="absolute top-24 right-4 md:top-32 md:right-16 z-30 hidden lg:flex animate-slide-in-right">
          <div className="px-5 py-3 rounded-lg bg-white/10 border border-white/20 backdrop-blur-md">
            <span className="text-white font-bold text-sm">Open Daily 5 AM – 11 PM</span>
          </div>
        </div>
        ...
        <div className="relative z-10 container mx-auto px-4 md:px-6 h-full flex flex-col justify-center pt-16 md:pt-16">
          <div className="max-w-5xl">
            {/* Tagline — stronger social proof */}
            <div className="flex items-center gap-4 mb-6 md:mb-8 animate-slide-up">
              <div className="px-4 py-1.5 md:px-5 md:py-2 border border-white/20 bg-white/10 text-white text-[11px] md:text-xs font-bold uppercase tracking-[0.2em] rounded-full flex items-center gap-2 backdrop-blur-md">
                <span className="text-dfw-red" aria-hidden="true">★</span> DFW&apos;s #1 Indoor Sports Facility
              </div>
            </div>

            {/* Headline — clear, emotional, SEO-rich */}
            ...
            {/* Visually hidden SEO subtitle */}
            <span className="sr-only">Premier Indoor Sports Facility in Fort Worth, TX — Cricket, Badminton, Soccer & Dodgeball</span>

            {/* Description & Buttons — clear CTA hierarchy */}
            <div className="flex flex-col lg:flex-row items-start lg:items-center gap-8 md:gap-20 animate-slide-up-delay-2">
              <p className="text-base md:text-xl text-gray-200 font-light max-w-xl leading-relaxed border-l-2 border-dfw-red pl-6 md:pl-8">
                More than a facility — a community. Whether you&apos;re picking up a racket for the first time or chasing a championship, you have a home here in Fort Worth.
              </p>

              <div className="flex flex-row gap-4 md:gap-5 w-full sm:w-auto">
                {/* Primary CTA — Book (highest intent) */}
                <a href="https://book.runswiftapp.com/facilities/dfw-indoor-sports/rentals" target="_blank" rel="noopener noreferrer" className="group relative whitespace-nowrap px-8 md:px-12 py-4 md:py-5 bg-dfw-red text-white text-sm font-bold uppercase tracking-widest overflow-hidden rounded-full shadow-lg hover:shadow-dfw-red/40 hover:-translate-y-1 transition-all duration-300 text-center flex justify-center items-center">
                  <span className="relative z-10 flex items-center gap-3">
                    Book a Court <ChevronRight size={18} className="group-hover:translate-x-1 transition-transform" aria-hidden="true" />
                  </span>
                </a>

                {/* Secondary CTA — Explore (lower intent, subtle) */}
                <Link href="/memberships" className="whitespace-nowrap px-6 md:px-8 py-4 md:py-5 border border-white/20 hover:bg-white hover:text-dfw-navy text-white text-sm font-medium uppercase tracking-widest transition-all rounded-lg backdrop-blur-md hover:shadow-lg hover:-translate-y-1 text-center flex items-center justify-center">
                  View Plans
                </Link>
              </div>
            </div>

            {/* Feature Strip — actionable info users actually need */}
            <div className="mt-12 md:mt-24 pt-8 md:pt-10 border-t border-white/10 grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 animate-fade-in-delay-3">
              {[
                { icon: Zap, text: "7 Sports" },
                { icon: Calendar, text: "Open 5AM – 11PM" },
                { icon: MapPin, text: "Fort Worth, TX" },
                ...
                <div key={idx} className="flex items-center gap-3 md:gap-4 text-gray-300 group cursor-default hover:text-white transition-colors duration-500">
                  ...
                  <span className="text-[11px] md:text-xs font-bold uppercase tracking-widest leading-tight">{item.text}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="absolute bottom-4 md:bottom-6 left-1/2 -translate-x-1/2 flex-col items-center gap-2 animate-bounce opacity-60 hidden md:flex">
          <span className="text-[11px] font-medium text-white/50 uppercase tracking-widest">Explore</span>
